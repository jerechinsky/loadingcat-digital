...
Latest release: **1.5.5**, under **yerex**. Adds an optional white cat on a black background when disconnected and a configurable extra delay for both visual and vibration alerts. Alerts are off by default; when enabled, three quick taps is the default pattern. No startup or reconnect buzz. Retains 12 spokes and nighttime animation pause. Every kick starts from the live seconds position, including repeated flicks; finishing or stopping a spin resumes the current seconds position. The GitHub account remains jerechinsky.

The extra disconnect delay is 0, 5, 10, 15, 30 or 60 seconds after Pebble reports the loss. The firmware itself filters disconnects for about 25 seconds; a watchface cannot shorten that interval. Reconnecting cancels a pending alert and restores the normal colors. The white-cat appearance is optional and off by default. Opening the face while already disconnected shows the chosen visual state without buzzing.
...
Version 1.5.5 builds for all seven platforms. Its verification report distinguishes fresh timing, settings and native checks from earlier unchanged layout evidence. The original author is prominently thanked and linked in the repository and listing. Use the matching `verification.json` for release checks.
...
3. Upload `loading-cat-digital.pbw`, version **1.5.5**. It contains aplite, basalt, chalk, diorite, emery, flint and gabbro builds. Keep its existing UUID for subsequent updates.
