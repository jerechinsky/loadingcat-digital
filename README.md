...
- **animation:** flick your wrist for a spin that slows down, then returns to the current seconds position. choose slow or fast, lasting 2, 3 or 4 seconds. default is fast, 3 seconds, 8 spokes. turn off both triggers to disable animation. speed, duration and the animation night pause only appear when a supported trigger is enabled.
- **nighttime:** separately pause seconds movement and wrist-flick/backlight animations during shared hours (initially 22:00–07:00). both night options start off. pausing seconds leaves the spinner visible and stops its seconds timer; movement resumes at the current time when night ends.
- **backlight trigger:** can also spin when the backlight wakes on Time 2, 2 Duo and Round 2 with compatible firmware. this includes the Back button and wrist-flick backlighting, even if the separate flick trigger is off.
