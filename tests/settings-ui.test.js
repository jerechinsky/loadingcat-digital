...
(async()=>{
 assert.equal(Object.keys(specs).length,15);
 ...
 async function checkStock(){
  assert.equal(await page.locator('input[data-manipulator-target],select[data-manipulator-target]').count(),15);
 ...
 assert.deepEqual(await options('SPOKES'),[['6','6'],['7','7'],['8','8'],['10','10']]);
 ...
 const expected={SECOND_HAND:0,GRAY_NOSE:0,SHOW_WEATHER:0,SHOW_SPINNER:0,NUMERAL_FONT:8,TIME_FORMAT:2,LEADING_ZERO:0,SPOKES:7,SPIN_MOTION:0,ANIMATE:0,FLICK_TRIGGER:0,LIGHT_TRIGGER:0,SPIN_LENGTH:2,FAHRENHEIT:1,WEATHER_INTERVAL:60};
