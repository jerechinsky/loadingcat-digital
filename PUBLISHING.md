...
Latest release: **1.5.4**, under **yerex**. Adds an optional phone disconnect alert with four vibration patterns and a Quiet Time override. Alerts are off by default; when enabled, three quick taps is the default pattern. No startup or reconnect buzz. Retains 12 spokes and nighttime animation pause. Every kick starts from the live seconds position, including repeated flicks; finishing or stopping a spin resumes the current seconds position. The GitHub account remains jerechinsky.
...
Version 1.5.4 builds for all seven platforms. Its verification report distinguishes fresh timing, settings and native checks from earlier unchanged layout evidence. The original author is prominently thanked and linked in the repository and listing. Use the matching `verification.json` for release checks.
...
3. Upload `loading-cat-digital.pbw`, version **1.5.4**. It contains aplite, basalt, chalk, diorite, emery, flint and gabbro builds. Keep its existing UUID for subsequent updates.
