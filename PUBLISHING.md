...
Version 1.5.6 builds for all seven platforms. Its verification report distinguishes fresh timing, settings and native checks from earlier unchanged layout evidence. The original author is prominently thanked and linked in the repository and listing. Use the matching `verification.json` for release checks.
...
3. Upload `loading-cat-digital.pbw`, version **1.5.9**. It contains aplite, basalt, chalk, diorite, emery, flint and gabbro builds. Keep its existing UUID for subsequent updates.
...
Version 1.5.7 adds **Weather location** (Phone location or Custom city) and a conditional **City** field with the example `Prague, CZ`. Phone location remains the default. Custom mode avoids GPS and caches city coordinates on the phone. Location changes clear the watch reading, and responses from a previous location are ignored. See `verification.json` for the fresh builds, settings, weather and native location-switch checks.

Version 1.5.8 accepts common country aliases (including USA and UK), selects the lookup language for non-Latin city names and tries the municipal suffix for short Japanese names. Live checks cover San Francisco, NYC, Praha, Kyiv and Lviv in Ukrainian, Beijing and Shanghai in Chinese, and Tokyo, Osaka and Kyoto in Japanese. This does not guarantee every alternate name in the external city database.

Version 1.5.9 adds optional reconnect vibration and a separate four-pattern choice. It starts off, with Short tap as its selected pattern. The shared Ignore Quiet Time switch covers disconnect and reconnect alerts. Connection changes use the existing OS subscription and add no polling or timers. The city hint explicitly labels its sample locations with "For example".
